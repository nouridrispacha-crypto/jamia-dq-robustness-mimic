...
def pollution_mnar_top(
    ...
    """
    MNAR diffus severity-weighted (binaire, comparable MCAR)

    - d % = d % EXACT de cellules manquantes (comme MCAR)
    - Sévérité définie par un indicateur binaire :
        norepinephrine ∈ {0,1}
    - Patients sous vasopresseur → probabilité plus élevée de valeurs manquantes
    ...
    # ------------------------------------------------------------
    # 2️⃣ Probabilité MNAR par patient (2 groupes)
    # ------------------------------------------------------------
    alpha = 5.0  # force de séparation

    p_severe = 1.0 / (1.0 + np.exp(-alpha / 2))   # sev == 1
    p_nonsev = 1.0 / (1.0 + np.exp(+alpha / 2))   # sev == 0

    p_row = np.where(sev == 1, p_severe, p_nonsev)
...
pollution_mnar_top.INTRODUCES_INVALID = False